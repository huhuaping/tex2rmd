...
processDisplayEqns <- function(tex){
  ...
  eqnPosBegin <- regexpr( "\\\\begin\\{(equation|equation\\*|gather|aligned)\\}", tex)
  eqnPosEnd <- regexpr( "\\\\end\\{(equation|equation\\*|gather|aligned)\\}", tex)
  ...
  if( length(eqnPosBegin) > 0 ){
    for( i in 1:length(eqnPosBegin)){
      ...
      #eqn <- gsub("\\\\(begin|end)\\{(equation|equation\\*|gather|aligned)\\}","",eqn)
      eqn <- gsub("\\\\label\\{[^\\}]*\\}","",eqn)
      eqn <- gsub("\\\\nonumber","",eqn)
      #eqn[1] <- paste0("$$", eqn[1])
